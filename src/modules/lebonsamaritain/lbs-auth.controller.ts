...
@Controller('lbs/auth')
export class LbsAuthController {
  ...
  async setup(@Body() body: { email: string; password: string; name?: string }) {
    ...
    try {
      ...
      const token = this.jwt.sign(
        { sub: admin.id, email: admin.email, role: 'LBS_ADMIN' },
        { expiresIn: '30d' },
      );
  ...
  // ── Refresh token (extend session 30j) ────────────────────────
  @Post('refresh')
  @UseGuards(LbsAdminGuard)
  @HttpCode(HttpStatus.OK)
  async refresh(@Request() req: any) {
    const token = this.jwt.sign(
      { sub: req.user.sub, email: req.user.email, role: 'LBS_ADMIN' },
      { expiresIn: '30d' },
    );
