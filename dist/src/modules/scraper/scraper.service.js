...
let ScraperService = ScraperService_1 = class ScraperService {
    ...
    async scrapeProduct(url) {
        if (!url.includes('aliexpress.com')) {
            throw new common_1.BadRequestException('URL AliExpress invalide');
        }
        ...
        try {
            ...
            try {
                const response = await axios_1.default.get(mobileUrl, { headers: HEADERS, timeout: 20000 });
                html = response.data;
            }
            catch {
                throw new common_1.BadRequestException('Impossible de récupérer la page AliExpress');
            }
        }
        ...
        $('script').each((_, el) => {
            ...
            if (!runParams && content.includes('"descriptionModule"')) {
                ...
                if (match) {
                    try {
                        runParams = { data: JSON.parse(match[1]) };
                    }
                    catch { }
                }
            }
        ...
        const name = titleModule?.subject ||
            $('h1[data-pl="product-title"]').text().trim() ||
            $('[class*="title--wrap"] h1').text().trim() ||
            $('h1').first().text().trim() ||
            'Produit AliExpress';
        const priceStr = priceModule?.minActivityAmount?.value ||
        ...
        images = images
            ...
            .slice(0, 10);
        const description = $('[id*="description"]').text().trim().slice(0, 2000) ||
    ...
    async importProduct(url, categoryId, markup = 30) {
        const scraped = await this.scrapeProduct(url);
        const salePrice = Math.round(scraped.price * (1 + markup / 100) * 655.957);
        let slug = (0, slugify_1.default)(scraped.name, { lower: true, strict: true });
